...
class RectTupleData():
    def __init__(self, len_tuple, DATA_PATH, n=N):
        ...
        try:
            ...
            for i in range(n):
                ...
                self._X.tofile(fid_X);
                fid_X.flush()
                self._Y.tofile(fid_Y);
                fid_Y.flush()
                self._Y_sup.tofile(fid_Y_sup);
                fid_Y_sup.flush()
            fid_X.close();
            fid_Y.close();
            fid_Y_sup.close();
    ...
    def next_batch(self, BATCH_SIZE):
        ...
        self._X = np.frombuffer(self._fid_X.read(4 * BATCH_SIZE * self._len_tuple * IM_SIZE * IM_SIZE),
                                dtype=np.float32)
